...
const localEmojiNames = computed(() => new Set(customEmojis.value.map(emoji => emoji.name)));
...
const segments = computed<Segment[]>(() => {
	...
	for (const match of value.matchAll(customEmojiRegex)) {
		...
		if (!emojiName.includes('@') && !localEmojiNames.value.has(emojiName)) {
...
async function openAutocomplete<T extends keyof CompleteInfo>(type: T, q: CompleteInfo[T]['query']) {
	...
	const { dispose } = popup(defineAsyncComponent(() => import('@/components/MkAutocomplete.vue')), {
		textarea: editorEl.value as any,
