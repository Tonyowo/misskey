		...
		super(meta, paramDef, async (ps, me) => {
			...
			if (ps.room) {
				...
				for (const message of packedMessages) {
					...
					message.hasUnreadMention = mentionStateMap[message.toRoomId!]?.hasUnreadMention ?? false;
					message.mentionMessageId = mentionStateMap[message.toRoomId!]?.mentionMessageId ?? null;
				}
			} else {
